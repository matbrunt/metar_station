    ...
    return DataFrameSchema(
        {
            ...
            "relh": Column(
                float, checks=[Check.less_than_or_equal_to(max_value=100.0)], nullable=True
            ),  # relative humidity (%)
            "drct": Column(float, checks=[Check.in_range(0.0, 360.0)], nullable=True),  # wind direction (degrees)
            "sknt": Column(
                float, checks=[Check.greater_than_or_equal_to(min_value=0.0)], nullable=True
            ),  # wind speed (kts)
            "alti": Column(float, checks=[Check.greater_than_or_equal_to(min_value=0.0)]),  # pressure altimeter (in)
            "mslp": Column(
                float, checks=[Check.greater_than_or_equal_to(min_value=0.0)], coerce=True, nullable=True
            ),  # sea level pressure (millibar)
            "vsby": Column(float, checks=[Check.in_range(0.0, 10.0)]),  # visbility (miles)
            "skyc\d": Column(  # noqa: W605
                str,
                checks=[Check.isin(CLOUD_COVERAGE_CODES, raise_warning=True)],
                coerce=True,
                nullable=True,
                regex=True,
            ),  # sky coverage
            "skyl\d": Column(  # noqa: W605
                float, checks=[Check.in_range(0.0, 12_000.0)], coerce=True, nullable=True, regex=True
            ),  # sky altitude (ft)
